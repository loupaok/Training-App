...
// Registered before /:id so "openfoodfacts"/"unsplash" never match as an :id.
router.get('/search/openfoodfacts', authorizeRole(['coach', 'admin']), async (req, res) => {
  const query = String(req.query.q || '').trim();
  if (!query) return res.json({ results: [] });

  try {
    const url = `https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(query)}&search_simple=1&action=process&json=1&page_size=5&fields=code,product_name,brands,image_front_url,nutriments`;
    const response = await fetch(url, { headers: { 'User-Agent': 'CoachApp - Food Library (contact: admin@example.com)' } });

    if (!response.ok) {
      return res.status(502).json({ message: 'Open Food Facts request failed', results: [] });
    }

    const data = await response.json();
    const results = (data.products || []).map((product) => ({
      code: product.code,
      name: product.product_name || 'Άγνωστο προϊόν',
      brand: product.brands || '',
      imageUrl: product.image_front_url || '',
      caloriesPer100g: product.nutriments?.['energy-kcal_100g'] ?? null,
      proteinPer100g: product.nutriments?.proteins_100g ?? null,
      carbsPer100g: product.nutriments?.carbohydrates_100g ?? null,
      fatsPer100g: product.nutriments?.fat_100g ?? null,
      fiberPer100g: product.nutriments?.fiber_100g ?? null,
    }));

    res.json({ results });
  } catch (error) {
    console.error(error);
    res.status(502).json({ message: 'Open Food Facts request failed', results: [] });
  }
});

router.get('/search/unsplash', authorizeRole(['coach', 'admin']), async (req, res) => {
  const query = String(req.query.q || '').trim();
  const accessKey = process.env.UNSPLASH_ACCESS_KEY;

  if (!accessKey) {
    return res.json({ results: [], message: 'Unsplash δεν έχει ρυθμιστεί (λείπει UNSPLASH_ACCESS_KEY).' });
  }
  if (!query) return res.json({ results: [] });

  try {
    const url = `https://api.unsplash.com/search/photos?query=${encodeURIComponent(query)}&per_page=4`;
    const response = await fetch(url, { headers: { Authorization: `Client-ID ${accessKey}` } });

    if (!response.ok) {
      return res.status(502).json({ message: 'Unsplash request failed', results: [] });
    }
    ...
    const results = (data.results || []).map((photo) => ({
      ...
      fullImageUrl: photo.urls?.regular,
    ...
    console.error(error);
    res.status(502).json({ message: 'Unsplash request failed', results: [] });
  }
});
