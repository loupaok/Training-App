...
const mealTitle = (meal: NonNullable<NutritionPlan>["meals"][number], index: number) => meal.title?.trim() || meal.name || `Γεύμα ${index + 1}`
const mealTotals = (foods: NonNullable<NutritionPlan>["meals"][number]["foods"]): MacroTotals => foods.reduce((totals, food) => ({ calories: totals.calories + numberValue(food.calories), protein: totals.protein + numberValue(food.protein_g), carbs: totals.carbs + numberValue(food.carbs_g), fats: totals.fats + numberValue(food.fat_g) }), { calories: 0, protein: 0, carbs: 0, fats: 0 })
...
function ClientNutritionView({ plan }: { plan: NonNullable<NutritionPlan> }) {
  ...
  const toggleItem = (key: string, isChecked: boolean) => setChecked((current) => ({ ...current, [key]: isChecked }))
    ...
    <ClientShell user={user} logout={logout} paymentApproved={subscriptionActive} active="dashboard">
      <main className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="nutrition">{nutritionPlan ? <ClientNutritionView plan={nutritionPlan} /> : <Card><CardContent className="py-12 text-center text-muted-foreground">Δεν υπάρχει διαθέσιμο πλάνο διατροφής.</CardContent></Card>}</TabsContent>
