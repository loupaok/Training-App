...
export default function ClientDashboard() {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      setError(err.message || 'Δεν φορτώθηκε το dashboard.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white px-6 py-4 shadow-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <div>
            <div className="text-sm font-bold text-red-600">Client Dashboard</div>
            <h1 className="text-2xl font-black">{user?.fullName || 'Πελάτης'}</h1>
          </div>
          <nav className="flex items-center gap-3">
            <button className="rounded-md border border-slate-200 px-4 py-2 font-bold">
              Ειδοποιήσεις {data?.unreadNotifications ? `(${data.unreadNotifications})` : ''}
            </button>
            <button onClick={logout} className="rounded-md bg-slate-950 px-4 py-2 font-bold text-white">Logout</button>
          </nav>
        </div>
      </header>
          ...
          <div className="space-y-6">
                      ...
                      <div key={day.id} className="rounded-lg border border-slate-200 p-4">
                        ...
                        <div className="mt-3 space-y-2">
                          {day.exercises.map((exercise) => (
                            <div key={exercise.id} className="rounded-md bg-slate-50 p-3 text-sm">
                  ...
                  <div>
                    ...
                    <div className="mt-5 space-y-3">
                      {(data.nutrition.meals || []).map((meal) => (
                        <div key={meal.id} className="rounded-lg border border-slate-200 p-4">
                          <h3 className="font-black">{dayNames[meal.day_of_week]} - {mealLabels[meal.meal_type]}</h3>
                          <div className="mt-3 space-y-2">
                            {meal.foods.map((food) => (
                              <div key={food.id} className="flex items-center justify-between rounded-md bg-slate-50 p-3 text-sm">
            ...
            {weeklyOpen && (
              <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-6">
                <form onSubmit={submitWeeklyUpdate} className="w-full max-w-2xl rounded-lg bg-white p-6 shadow-2xl">
                  ...
                  <div className="mt-5 grid gap-4 md:grid-cols-2">
                    ...
                    <label className="block">
                      <span className="text-sm font-bold text-slate-700">Φωτογραφίες προόδου (max 3)</span>
                      <input type="file" accept="image/*" multiple onChange={(event) => setWeeklyForm({ ...weeklyForm, photos: Array.from(event.target.files || []).slice(0, 3) })} className="mt-2 block w-full rounded-md border border-slate-200 p-3" />
                    </label>
                    <label className="md:col-span-2">
                      <span className="text-sm font-bold text-slate-700">Σημειώσεις / ερωτήσεις προς τον coach</span>
                      <textarea value={weeklyForm.notes} onChange={(event) => setWeeklyForm({ ...weeklyForm, notes: event.target.value })} className="mt-2 min-h-32 w-full rounded-md border border-slate-200 p-3 outline-none focus:border-red-300" />
                    </label>
                  </div>
                  <div className="mt-6 flex justify-end gap-3">
                    <button type="button" onClick={() => setWeeklyOpen(false)} className="h-11 rounded-md border border-slate-200 px-5 font-bold">Άκυρο</button>
...
function WeeklyUpdateCard({ data, onOpen }) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <div className="text-sm font-bold text-slate-500">Εβδομαδιαίο Update</div>
...
function EmptyState({ text }) {
  return <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-8 text-center font-bold text-slate-500">{text}</div>;
}
...
function Input({ label, value, onChange, required = false }) {
  return (
    <label className="block">
      ...
      <input value={value} onChange={(event) => onChange(event.target.value)} required={required} className="mt-2 h-11 w-full rounded-md border border-slate-200 px-3 outline-none focus:border-red-300" />
    </label>
  );
}
...
function Score({ label, value, onChange }) {
  return (
    <label className="block">
      <span className="text-sm font-bold text-slate-700">{label}</span>
      <select value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 h-11 w-full rounded-md border border-slate-200 px-3 outline-none focus:border-red-300">
